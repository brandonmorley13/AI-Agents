...
- **Invocable skill workflows** (`skills/`) — slash-command entry points that wire a subagent (or several) to a specific trigger: scaffold a feature, review a diff, run a pre-launch check.
...
1. Copy `agents/` and `skills/` into your own `.claude/` directory.
